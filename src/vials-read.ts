...
export const IsoFormVis = SimpleReadsVis;
...
SimpleReadsVis.prototype.build = function ($parent) {
  ...
  function createLocalGui() {
    ...
    guiGroupBy.append("rect").attr({
      ...
      "click": function () {
        ...
        for (let meta in metaGroups) {
          joinGroups(metaGroups[meta].map(function (sample) {
            return getGroupFromSample(sample).groupID
          }));
        }
      }
    })
  ...
  function updateCrosshair(event, x) {
    ...
    if (x < 0 || x > that.axis.getWidth()) {
      ...
      svg.selectAll(".crosshairValue")
        .text(function (d) {
          return dataFuncs[dataType].valueAtIndex(d, x)
        })
  ...
  var sampleScaleY = function (x) {
    return x * (sampleHeight + 3)
  };
  var scaleXScatter;
  var heightScale;

  function getMetaFromSample(sample) {
    for (let curSample in sampleInfo) {
      if (curSample == sample) {
  ...
  function zipData(sampleData) {
    return sampleData[0].weights.map(function (s0, i) {
      return sampleData.map(function (s) {
        return s.weights[i]
      });
    })
  }
    ...
    "TCGA": {
      "mean": function (zipped) {
        function meanData(exonWeights) {
          return d3.mean(exonWeights, function (d: any) {
            return d.weight
          });
        }

        var mappedReads = this.mapReads(this.mapExons(zipped, meanData));
        var mean = d3.svg.line<any>()
          .x(function (d, i) {
      ...
      "stdDev": function (zipped) {
        function stdData(exonWeights) {
          return exonWeights.map(function (e) {
            return e.weight
          });
        }

        var stdDev = d3.svg.area<any>()
          .x(function (d, i) {
            return that.axis.genePosToScreenPos(d.pos);
          })
          .y0(function (d) {
      ...
      "mapReads": function (exons) {
        ...
        exons.forEach(function (e) {
          let exonCoords = [
            {
              "pos": e.start,
              "weight": e.weight.length > 1 ? e.weight.map(function () {
                return 0
              }) : 0
            },
            {
  ...
  function drawSamples(group) {
     ...
     * =========================
     * */
    var abundance = g.selectAll(".abundance").data(group.data, function (d) {
      return d.sample
    });
    ...
    abundance.select(".abundanceGraph").attr({
      "class": function (d) {
        return "abundanceGraph sample" + cleanSelectors(d.sample)
      },
      "d": function (d) {
        return dataFuncs[dataType].line(d)
      }
    })
  ...
  function repositionGroups() {
    ...
    var groupScaleY = function (x, noSamplesBefore) {
      return noSamplesBefore * (sampleHeight + 3) + 20
    };
    ...
    groups.transition().attr({
      "transform": function (d, i) {
        let groupPos = groupScaleY(i, noSamplesBefore);
  ...
  function redrawLineGroups(group) {
    ...
    let buttonHeight = sampleScaleY(0);
  ...
  function summarizeData(group) {
    ...
    var groupText = group.groupID.meta != "none" ? " " + group.groupID.meta : group.groupID.samples.map(function (s) {
      return " " + s
    });
    groupText = "Group" + groupText;
    var titleText = "Group: \nMeta: " + group.groupID.meta + "\nSamples: "
      + group.groupID.samples.map(function (s) {
        return "\n" + s
      });
  ...
  function drawGroups() {
    var group = gIso.selectAll(".sampleGroup")
      .data(sampleGroups, function (group) {
        return group.data.map(function (d) {
          return d.sample
        })
      })
  ...
  function ungroup(group) {
    if (group.samples.length > 1) {
      ...
      sampleGroups = newGroups.concat(sampleGroups.filter(function (curGroup) {
        return curGroup != group
      }));
      event.fire("groupingChanged", newGroups.map(function (group) {
  ...
  function joinGroups(groupIDs) {
    var combinedData = groupIDs.reduce(function (newGroup, groupID) {
      var group = getGroup(groupID);
      group.samples.forEach(function (s) {
        event.fire("sampleSelect", s, false)
      })
    ...
    var meta = {};
    combinedData.samples.forEach(function (s) {
      meta[getMetaFromSample(s)] = null
    });
    meta = d3.keys(meta);
    ...
    sampleGroups = [newGroup].concat(sampleGroups.filter(function (group) {
  ...
  function updateData() {
    ...
    that.data.getGeneData(updatedProject, curGene).then(function (sampleData) {
      ...
      if (sampleGroups === undefined || updatedProject != curProject) {
        ...
        event.fire("groupingChanged", sampleGroups.map(function (group) {
          return group.groupID
        }), [])
      }
